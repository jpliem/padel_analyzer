...
- **VLM audit is pointing-based**: `track_audit.py` default is `--mode pointing` with `qwen3-vl:2b-instruct` — the VLM locates the ball on a full-res crop (0-1000 normalized coords), the code computes the verdict from distance. Judgment prompts measured unusable on small models (fixed-bias answers, `scripts/eval_vlm_auditor.py`). Use `-instruct` Ollama tags only; thinking variants return empty structured output.
- **Player re-ID**: `OsnetAppearanceEncoder` (vendored `cv/vendor/osnet.py`, weights `backend/models/osnet_x0_25_msmt17.pth`) is the production encoder; HSV histogram is the fallback and measured unable to separate similar kits.
- **Session history**: `docs/changelog-2026-07-19-research-auditor-reid.md` (SOTA research, auditor redesign, OSNet, accuracy-fixes port, next actions). `docs/changelog-2026-07-18-production-hardening.md` records the hardening pass, measured baselines, and remaining gaps. `docs/multicam-3d-progress.md` records why the architecture is single-camera + physics (multicam is the validation harness, not the product).
